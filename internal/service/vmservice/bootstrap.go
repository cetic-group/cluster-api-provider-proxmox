...
func getVirtualNetworkDevices(_ context.Context, _ *scope.MachineScope, networkSpec infrav1.NetworkSpec, data []network.NetworkConfigData) ([]network.NetworkConfigData, error) {
	...
	for _, device := range networkSpec.VRFs {
		...
		for _, child := range device.Interfaces {
			found := false
			for j := range data {
				if data[j].Name == string(child) || data[j].ProxName == child {
					// Store the member interface's name; the reverse (Parent)
					// link is derived by name in network.Network.
					config.Children = append(config.Children, data[j].Name)
					found = true
				}
			}
			if !found {
