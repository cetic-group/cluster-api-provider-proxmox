...
// network.Network) and then the cloud-init renderer's own checks for the subset
// of the stack it implements.
...
// validate holds the cloud-init-specific checks: every ethernet link this
// renderer emits needs a MAC address to match on and a valid address (unless
// it is configured for DHCP). Structural, renderer-agnostic checks live in
